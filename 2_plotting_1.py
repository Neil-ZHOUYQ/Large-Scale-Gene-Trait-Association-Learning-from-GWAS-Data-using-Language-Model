...
root_path = "/mnt/home/zhouyuqi/bert/out/nlp/biomedbert"  # Adjust this to your path
run_name = "250429_113915_E9_B32_LR2e-05_WD0.01"  
...
def plot_training_loss():
    ...
    try:
        ...
        ax.plot(steps, train_loss, color='#1f77b4', linewidth=2)
        
        # Add moving average for trend visualization
        window_size = min(100, max(10, len(train_loss) // 20))
        if len(train_loss) > window_size:
            moving_avg = pd.Series(train_loss).rolling(window=window_size).mean().iloc[window_size-1:].values
            ax.plot(range(window_size, len(train_loss)+1), moving_avg, 
        ...
        if 'num_train_epochs' in run_name:
            # Try to extract epochs and batch size from run name
            ...
            for part in parts:
                if part.startswith('E') and part[1:].isdigit():
                    ...
                    for e in range(1, epochs):
                        ...
                        ax.text(e*steps_per_epoch, max(train_loss), f'Epoch {e}', 
                               ha='center', va='bottom', alpha=0.7)
        
        if 'moving_avg' in locals():
            ax.legend()
