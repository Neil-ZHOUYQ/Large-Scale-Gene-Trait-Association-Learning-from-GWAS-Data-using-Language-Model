...
gwas_data_fn = "/mnt/home/zhouyuqi/bert/out/nlp/biomedbert/datasets/case/all_gene_trait_assoc.GWAS_ASD.tsv" 
...
output_analysis_dir = os.path.join(os.path.dirname(gwas_data_fn), "gene_tokenization_analysis_all")
...
def analyze_gene_tokenization():
    ...
    if num_unknown > 0:
        print(f"\nFirst few known genes: {known_genes[:10]}...")
        # Save the full list of unknown genes
        known_genes_file = os.path.join(output_analysis_dir, "known_genes.txt")
        try:
            with open(known_genes_file, 'w') as f:
                for gene in known_genes:
                    f.write(f"{gene}\n")
            print(f"Full list of {num_known} known genes saved to: {known_genes_file}")
        except Exception as e:
