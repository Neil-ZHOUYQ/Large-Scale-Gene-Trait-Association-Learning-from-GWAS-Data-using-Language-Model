...
def generate_comparison_plots(original_results, finetuned_results, test_texts, test_labels):
    ...
    try:
        ...
        axes[0].set_title('Original BiomedBERT Confusion Matrix')
        ...
        axes[1].set_title('Finetuned BiomedBERT Confusion Matrix')
        ...
        plt.plot(fpr_original, tpr_original, color='blue', lw=2, 
                 label=f'Original BiomedBERT (AUC = {roc_auc_original:.3f})')
        ...
        plt.plot(fpr_finetuned, tpr_finetuned, color='red', lw=2, 
                 label=f'Finetuned BiomedBERT (AUC = {roc_auc_finetuned:.3f})')
        ...
        plt.plot(recall_original, precision_original, color='blue', lw=2,
                 label=f'Original BiomedBERT (AUC = {pr_auc_original:.3f})')
        ...
        plt.plot(recall_finetuned, precision_finetuned, color='red', lw=2,
                 label=f'Finetuned BiomedBERT (AUC = {pr_auc_finetuned:.3f})')
        ...
        plt.hist([original_probs[i] for i in range(len(test_labels)) if test_labels[i] == 1], 
                alpha=0.4, bins=20, color='blue', label='Original: True Associations')
        plt.hist([original_probs[i] for i in range(len(test_labels)) if test_labels[i] == 0], 
                alpha=0.4, bins=20, color='cyan', label='Original: False Associations')
        
        # Finetuned model prediction distributions
        plt.hist([finetuned_probs[i] for i in range(len(test_labels)) if test_labels[i] == 1], 
                alpha=0.4, bins=20, color='red', label='Finetuned: True Associations')
        plt.hist([finetuned_probs[i] for i in range(len(test_labels)) if test_labels[i] == 0], 
                alpha=0.4, bins=20, color='orange', label='Finetuned: False Associations')
        ...
        plt.bar(x - width/2, original_values, width, label='Original BiomedBERT', color='blue', alpha=0.7)
        plt.bar(x + width/2, finetuned_values, width, label='Finetuned BiomedBERT', color='red', alpha=0.7)
...
def compare_models():
    ...
    original_model = BertForSequenceClassification.from_pretrained(original_model_name, num_labels=2)
    ...
    finetuned_model = BertForSequenceClassification.from_pretrained(finetuned_model_dir, num_labels=2)
