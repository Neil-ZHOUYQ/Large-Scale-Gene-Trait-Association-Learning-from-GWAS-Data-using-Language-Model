...
dataset_fn = os.path.join(root_path, "datasets", "case", "case_train_dataset.GWAS_ASD.20250428.txt.gz")
...
#Dateset: contain information about the examples.the examples can be acccessed from disk by mapping the information.
#This helps to handle datasets that larger than available RAM
#Example of using information(representation to map the training examples):
#dataset['train'][0]
...
def tokenize_function(batch):
    # Tokenize the batch of sentences directly
    # The tokenizer handles batching efficiently and returns dict of lists
    return tokenizer(
        batch['text'],
        padding='max_length', # Or False/True if using dynamic padding in collator
        truncation=True, #deal with sequences longer than the max_length
        max_length=512, #max length of input
        return_special_tokens_mask=True 
        # Needed by DataCollatorForLanguageModeling if mlm=True
...
tokenized_dataset = dataset.map(
    ...
    # num_proc=4 # Optional: Use multiple processes if dataset is large
)
print("Tokenization complete.")
print(tokenized_dataset) # Check the structure

# --- Instantiate Data Collator (Used later by Trainer) ---
# This will be passed to the Trainer, not used inside .map()
data_collator = DataCollatorForLanguageModeling(
    tokenizer=tokenizer,
    mlm=True, # Masked Language Modeling objective
    mlm_probability=0.15
)


# def prepare_data(corpus):
#     instances = [tokenizer(sent, padding='max_length', truncation=True, max_length=512, return_special_tokens_mask=True) for sent in corpus['text']]
#     data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=True, mlm_probability=0.15)
#     dataset = data_collator(instances)
#     return dataset

# tokenized_dataset = dataset.map(prepare_data, batched=True, remove_columns=dataset["train"].column_names)
...
model = BertForMaskedLM.from_pretrained(model_name, 
                                        attn_implementation="sdpa")#attention mechanism calculation
...
lr_scheduler = get_scheduler(
    ...
    #!! change from num_warmup_steps = 1500 
...
with open(trainingloss_file, 'w') as file:
